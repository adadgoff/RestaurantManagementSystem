...
4. Нет Exceptions в Service, так как вся настройка в Security. В других случаях использовать принцип: `Easier to ask for
   forgiveness than permission`;
5. Писать комментарии в коде на английском;
6. Все скрытые поля в html находятся в начале формы.
7. В Сервисах работа с dto-шками;
8. Использовал только `@PostMapping`, так как html-формы не позволяют делать другой вид запроса, кроме `GET` и `POST`;
9. **Не упоминать о Программистском клубе...**

## Варианты улучшения проекта

- JWT-аутентификация;
- Добавить возможность смены пароля пользователям;
- Добавить двухфакторную аутентификацию по почте (Java Mail Sender);
- Поддержка разных языков проекта. По идее надо все строки вынести в отдельные переменные и их использовать в
  зависимости от языка;
- Убрать `@Autowired` (не смог без него, так как JPARepository были `null`, так и не понял, как это исправить);
- Добавить тесты.
...
- [x] Администратор может добавлять и удалять блюда из меню, а также устанавливать их количество, цену и сложность
