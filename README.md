...
У заказа есть только два состояния:

- `Готовится`;
- `Готов`.

1. USER делает заказ, выбирая позиции меню. В БД ничего не записывается, а создается поток. Количество потоков (поваров)
   задается в настройках (`GLOBAL_VARIABLES`). Статус заказа становится `Готовится`.
2. Посетитель имеет возможность, пока все блюда не приготовятся (то есть должно пройти время приготовления всех блюд):
    - Добавить блюдо;
    - Отменить заказ;
